...
get Terms(){return this.FormNotLogged.get('Terms')!}
  ...
  private initMap(): void {
    this.map = L.map('map', {
      center: [this.itAcademyMarker.geometry.coordinates[0], this.itAcademyMarker.geometry.coordinates[1]],
      zoom: 15
    });
  ...
  private addMarker(map: L.Map, geoJSON: any): void {
    ...
    console.log('lat', lat);
    console.log('lon', lon);
    const marker = L.marker([lat, lon], { icon: iconDefault }).addTo(map);
